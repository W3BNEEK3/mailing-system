<?php
/**
* resources/compose/index.php
*
* The main compose page. Layout: a single-column card on desktop, full-screen
* on mobile. The compose form is split across sub-partials.
*
* @var \App\Models\EmailTemplate[]  $templates      All templates for the selector
* @var \App\DTOs\RenderContext      $globalContext  Global branding for colour pickers
* @var \App\Models\EmailDraft|null  $draft          Pre-loaded draft (or null for fresh compose)
*/
...
<div class="max-w-4xl mx-auto">
    ...
    <div id="compose-form">
        ...
        <input type="hidden" id="draft-id-input" name="draft_id"
        value="<?= $draft ? $draft->id : '' ?>">

        <input type="hidden" id="template-id-input" name="template_id"
        value="<?= $draft?->templateId ?? '' ?>">

        <input type="hidden" id="email-logo-path-input" name="email_logo_path"
        value="<?= e($draft?->emailLogoPath ?? '') ?>">

        <input type="hidden" id="primary-color-input"   name="primary_color"
        value="<?= e($draft?->primaryColor ?? '') ?>">
        <input type="hidden" id="secondary-color-input" name="secondary_color"
        value="<?= e($draft?->secondaryColor ?? '') ?>">

        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">

            <?= component('compose/_toolbar', [
                'templates' => $templates,
            ...
            <div class="flex items-center justify-between px-6 py-4 bg-slate-50 border-t border-slate-100">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button
                        ...
                        hx-swap="innerHTML"
                        hx-on::after-request="openPreviewModal(event)"
                        >
                        <i class="bi bi-eye"></i>
                        Preview
                    </button>
...
<script>
    document.addEventListener('draftSaved', function (event) {
        ...
        if (draftId) {
            document.getElementById('draft-id-input').value = draftId;
        }
    });
    ...
    function populateSendSummary() {
        ...
        if (summary) {
            ...
            + (recipients.length > 0
                ? '<span class="text-slate-600">' + recipients.length + ' recipient'
                + (recipients.length > 1 ? 's': '') + '</span>': '<span class="text-red-500">No recipients added</span>')
            + '</p>'
            + '<p><span class="font-medium text-slate-800">Subject: </span>'
            + escapeHtml(subject)
            + '</p>'
            + '</div>';
        }
