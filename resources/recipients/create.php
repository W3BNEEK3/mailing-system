...
<div class="mx-auto max-w-2xl px-4 py-8">
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <form method="POST" action="/recipients" class="px-6 py-6 space-y-5">
            <?= csrf_field() ?>
            ...
            <div class="flex flex-col gap-1">
                ...
                <input type="text" id="tags" name="tags" value="<?= e(old('tags', '')) ?>" placeholder="Clients, Newsletter, VIPs"
                ...
                <p class="text-xs text-slate-500">
                    ...
                    <?php if (!empty($groups)): ?>
                        ...
                        <?php foreach ($groups as $g): ?>
                            <button type="button" class="underline text-blue-600 hover:text-blue-700" onclick="addTag('<?= e(addslashes($g->name)) ?>')"><?= e($g->name) ?></button><?= ($g !== end($groups)) ? ',' : '' ?>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </p>
            </div>
            ...
            <div class="flex flex-col gap-1">
                ...
                <textarea id="notes" name="notes" rows="3" placeholder="Any notes about this contact…"
                          class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm placeholder-slate-400 resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"><?= e(old('notes', '')) ?></textarea>
            </div>
