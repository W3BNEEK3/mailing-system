...
<div class="mx-auto max-w-2xl px-4 py-8">
    ...
    <?php if ($recipient->isSuppressed()): ?>
        <div class="mb-5 flex items-center gap-3 rounded-xl border border-red-200 bg-red-50 px-5 py-3.5 text-sm text-red-800">
            ...
            <span>
                ...
                <button type="button" class="ml-2 underline font-medium hover:no-underline"
                        hx-post="/recipients/<?= (int) $recipient->id ?>/unsuppress"
                        hx-headers='{"X-CSRF-Token": "<?= e(csrf_token()) ?>"}'
                        hx-target="body" hx-swap="none" hx-on::after-request="window.location.reload()">
    ...
    <div class="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <form method="POST" action="/recipients/<?= (int) $recipient->id ?>" class="px-6 py-6 space-y-5">
            <?= csrf_field() ?>
            ...
            <div class="flex flex-col gap-1">
                ...
                <input type="text" id="tags" name="tags" value="<?= e(old('tags', $currentTags)) ?>" placeholder="Clients, Newsletter, VIPs"
                       class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1">
            ...
            <div class="flex flex-col gap-1">
                ...
                <textarea id="notes" name="notes" rows="3"
                          class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-800 shadow-sm resize-y focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1"><?= e(old('notes', $recipient->notes ?? '')) ?></textarea>
            </div>

            <div class="flex items-center justify-between pt-2 border-t border-slate-100">
                <a href="/recipients" class="text-sm text-slate-500 hover:text-slate-700">Cancel</a>
                <button type="submit" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-blue-700 transition">
